...
FONT_PATH = os.environ.get(
    "JetBrainsMono-Bold",
    os.path.join(HERE, "..", "assets", "fonts", "JetBrainsMono-Bold.ttf")
)

FONT_INDEX = 0
FONT_INDEX = int(os.environ.get("JetBrainsMono-Bold_INDEX", 2))   # face within a .ttc
...
TEXT = os.environ.get("WORDMARK_TEXT", "OMER")
...
def build_shell():
    ...
    for _ in range(40):                       # shrink until one line fits a sane raster
        try:
            ...
            # try common fallbacks on Linux systems
            fallback_paths = [
    ...
    # support both FreeTypeFont (getlength) and older bitmap fonts (getsize)
    def _getlength(s):
    ...
    def line_w(s):
        return sum(_getlength(c) for c in s) + track * (len(s) - 1)
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(
        f'<text x="{PAD}" y="{TITLEBAR_H/2 + 4:.0f}" '
        f'fill="{TITLE_TEXT}" font-size="12">'
        f'Windows Terminal'
        f'</text>'
    )

    p.append(
        f'<text x="{canvas_w-70:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" '
        f'fill="{TITLE_TEXT}" font-size="12" text-anchor="start">'
        f'— □ ✕'
        f'</text>'
    )
...
def main():
    ...
    out = a.out or os.path.join(HERE, "..", f"omer-wordmark-{a.mode}.svg")
