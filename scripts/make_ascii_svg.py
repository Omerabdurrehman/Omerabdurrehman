...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "omer-ascii.svg")
...
parts.append(
    f'<text x="{PAD}" y="{TITLEBAR_H/2 + 4}" '
    f'fill="{TITLE_TEXT}" font-size="13">'
    f'Windows Terminal'
    f'</text>'
)

parts.append(
    f'<text x="{CANVAS_W-70}" y="{TITLEBAR_H/2 + 4}" '
    f'fill="{TITLE_TEXT}" font-size="13">'
    f'— □ ✕'
    f'</text>'
)
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'omer@github:~$ whoami <tspan fill="{INK}">Omer Abdur Rehman</tspan></text>')
